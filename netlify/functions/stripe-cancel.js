...
exports.handler = async (event) => {
  ...
  if (currentSubscription.status === 'canceled') {
    ...
    addActivity(user, 'plan', 'plan_already_canceled_usd', { plan: planLabel });
  ...
  user.stripeLastEventAt = Math.floor(Date.now() / 1000);
  // A data (dateIso) é formatada no idioma/localidade certos no painel.html
  // no momento da renderização — não fixamos mais o formato pt-BR aqui.
  if (periodEndIso) {
    addActivity(user, 'plan', 'plan_canceled_period_end_usd', { plan: planLabel, dateIso: periodEndIso });
  } else {
    addActivity(user, 'plan', 'plan_canceled_period_end_generic_usd', { plan: planLabel });
  }
