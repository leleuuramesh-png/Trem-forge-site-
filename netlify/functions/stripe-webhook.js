...
async function updateUserPlan(email, updates, activity, eventCreated) {
  ...
  if (changed && activity) {
    addActivity(user, 'plan', activity.key, activity.params);
  }
...
async function handleCoinsPaymentCompleted(session) {
  ...
  addActivity(user, 'coins', 'coins_purchased_usd', { coins: purchase.coins, package: purchase.package });
...
exports.handler = async (event) => {
  ...
  if (stripeEvent.type === 'checkout.session.completed') {
    ...
    await updateUserPlan(email, {
      ...
      planCancelAt: null,
    }, { key: 'plan_confirmed_usd', params: { plan: planLabel } }, stripeEvent.created);
    return json(200, { ok: true });
  }
  ...
  if (stripeEvent.type === 'customer.subscription.updated' || stripeEvent.type === 'customer.subscription.deleted') {
    ...
    // A data (quando existir) é formatada no idioma certo pelo painel.html
    // no momento da renderização — aqui só passamos o ISO puro.
    let activity;
    if (isDeleted) {
      activity = { key: 'plan_ended_usd', params: { plan: planLabel } };
    } else if (newStatus === 'active' && cancelAtPeriodEnd) {
      activity = planCancelAt
        ? { key: 'plan_canceled_period_end_usd', params: { plan: planLabel, dateIso: planCancelAt } }
        : { key: 'plan_canceled_period_end_generic_usd', params: { plan: planLabel } };
    } else if (newStatus === 'active') {
      activity = { key: 'plan_confirmed_usd', params: { plan: planLabel } };
    } else if (newStatus === 'paused') {
      activity = { key: 'plan_paused_usd', params: { plan: planLabel } };
    } else if (newStatus === 'canceled') {
      activity = { key: 'plan_canceled_usd', params: { plan: planLabel } };
    } else if (newStatus === 'pending') {
      activity = { key: 'plan_pending_usd', params: { plan: planLabel } };
    }
