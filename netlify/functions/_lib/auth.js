...
// Grava uma entrada no feed de "Atividade recente". `key` é a chave de
// tradução em i18n/*.json (dentro de painel.activity.log.*) e `params` são
// os valores dinâmicos que o front-end insere no template (ex.: {plan},
// {name}, {streak}). Não gravamos mais o texto final em português aqui —
// isso é o que permite o painel.html renderizar cada item no idioma ativo
// do usuário em vez de travar tudo em pt-BR.
function addActivity(user, type, key, params) {
  user.activity.unshift({ type, key, params: params || null, at: new Date().toISOString() });
...
function awardBadge(user, badgeId) {
  ...
  addActivity(user, 'badge', 'badge_unlocked', { badgeId });
  return true;
}
...
function touchDailyVisit(user) {
  ...
  if (isFirstVisit) {
    ...
    addActivity(user, 'login', 'login_first');
  } else {
    addActivity(user, 'login', 'login_streak', { streak: user.streak });
  }
...
async function applyReferral(newUser, refCode) {
  ...
  addActivity(referrer, 'referral', 'referral_joined', { name: newUser.name || newUser.email });
  await users.setJSON(referrerEmail, referrer);

  newUser.coinsBalance += 20;
  newUser.referredBy = referrerEmail;
  addActivity(newUser, 'referral', 'referral_bonus');
  return true;
}
