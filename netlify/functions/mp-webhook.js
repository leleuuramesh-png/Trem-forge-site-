...
async function handlePaymentNotification(paymentId, accessToken) {
  ...
  addActivity(user, 'coins', 'coins_purchased_brl', { coins: purchase.coins, package: purchase.package });
...
exports.handler = async (event) => {
  ...
  if (changed) {
    const activityKeys = {
      active: 'plan_confirmed_brl',
      paused: 'plan_paused_brl',
      canceled: 'plan_canceled_brl',
      pending: 'plan_pending_brl',
    };
    if (activityKeys[newStatus]) {
      addActivity(user, 'plan', activityKeys[newStatus], { plan: planLabel });
    } else {
      addActivity(user, 'plan', 'plan_status_updated', { status: newStatus });
    }
  }
