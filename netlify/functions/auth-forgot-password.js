...
// Envia o código de redefinição por SMS via Zenvia. Mesmo padrão de erro
...
async function sendResetSMS(phone, code) {
  ...
  if (!token) {
    throw new Error('ZENVIA_API_TOKEN não configurada nas variáveis de ambiente.');
  }

  const from = process.env.ZENVIA_SMS_FROM || 'TremForge';
  ...
  const res = await fetch('https://api.zenvia.com/v2/channels/sms/messages', {
    ...
    body: JSON.stringify({
      ...
      contents: [
        {
          type: 'text',
          text: `Trem Forge: seu codigo de redefinicao de senha e ${code}. Valido por 15 minutos.`,
        },
      ],
    }),
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(`Falha ao enviar SMS via Zenvia (${res.status}): ${errText}`);
  }
}
